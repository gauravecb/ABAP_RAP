sap.ui.define([
	...
	return Controller.extend("MDMZCUSTOMER.controller.changeCustomer", {
		onInit: function() {
		...
		_handleRouteMatched: function(oEvent) {
			if (oEvent.getParameter("name") === "changeCustomer") {
				this.TaskInboxVariable = this.getOwnerComponent().getComponentData().startupParameters.TASK;
		...
		oneTimeHeaderRead: function(oData) {
			...
			var modl = sap.ui.getCore().getModel("searchExtSubTableModelSet").getData().ArrayExtIdent;
			var oGoldenRecId = oSelValue1Gold.results;
			var system = oSelValue1.System;
			var localid = oSelValue1.LocalID;
			//header data
			var hdr = {
				CompnyCodeKey: oData.Bukrs ? oData.Bukrs : "",
				SalesOrgKey: oData.Vkorg ? oData.Vkorg : "",
				DistribChnnlKey: oData.Vtweg ? oData.Vtweg : "",
				DivisionKey: oData.Spart ? oData.Spart : "",
				System: system ? system : "",
				AccountGrpKey: oData.Ktokd ? oData.Ktokd : "",
				AccountGrpDesc: (modl !== undefined && modl.applnRoleRowid !== undefined && modl.applnRoleRowid.roleDescription !== undefined) ?
					modl.applnRoleRowid.roleDescription : "",
				AppId: localid ? localid : "",
				GoldenRecordId: oGoldenRecId ? oGoldenRecId : "",
				CompnyCodeKeyEnable: false,
				SalesOrgKeyEnable: false,
				DistribChnnlKeyEnable: false,
				DivisionKeyEnable: false,
				SystemEnable: false,
		...
		readHdrAndCustBusAreaCreateEmpty: function(Data) {
			...
			var data = {
				...
				TaxData: {
					...
					"CFOP_CategoryDesc": oResultsTax.CfopCategoryDesc ? oResultsTax.CfopCategoryDesc.toString().trim() : "",
					"ICMS_Law": oResultsTax.IcmsLaw ? oResultsTax.IcmsLaw.toString().trim() : "",
					"ICMS_LawDesc": oResultsTax.IcmsLawDesc ? oResultsTax.IcmsLawDesc.toString().trim() : "",
					"IPI_Law": oResultsTax.IpiLaw ? oResultsTax.IpiLaw.toString().trim() : "",
					"IPI_LawDesc": oResultsTax.IpiLawDesc ? oResultsTax.IpiLawDesc.toString().trim() : "",
			...
			for (var v = 0; v < oVatDataResults.length; v++) {
				...
				if (oResults[0] !== undefined && oResults[0].CountryCode === countKey) {
					...
					data.TaxData.VatCountDesc = oVatDataResults[v].Countryname ? oVatDataResults[v].Countryname.toString().trim() : "";
				} else {
			...
			statusDataTabModel.getData().StatusData.DeletionBlock = true;
		...
		onChnageIconTabBar: function(oEvent) {
			...
			if (hdrModelData.AccountGrpKey.toString().trim() === "" || (hdrModelData.CompnyCodeKey.toString().trim() === "" && (hdrModelData.SalesOrgKey
			...
			//read first time Text Note

			/*	if ((tabChangeMdl.getData().SelecteIconTab === "ID_OP_TEXT_NOTE" || tabChangeMdl.getData().SelecteIconTab === "ID_OP_UPLOD_DOC") &&
					!this.firstTimeReadTextNoteIconTab) {
					this.readTextNotesIndAllButtonbasedOnOneF4ToNextF4 = true;
					this.firstTimeReadTextNoteIconTab = new Date();
					this.readTextNotesApiFirst();
				}
				//read f4 values based on first f4
				if ((tabChangeMdl.getData().SelecteIconTab === "ID_OP_TEXT_NOTE" || tabChangeMdl.getData().SelecteIconTab === "ID_OP_UPLOD_DOC") &&
					!this.readTextNotesIndAllButtonbasedOnOneF4ToNextF4) {
					this.readTextNotesIndAllButtonbasedOnOneF4ToNextF4 = true;
					this.bindTextNoteF4ValuesBasedOnFirstF4TextNotes();
					this.bindTextNoteF4ValuesBasedOnFirstF4Classific();
				}

				//each time call
				if (tabChangeMdl.getData().SelecteIconTab === "ID_OP_CON_DET" && !this.readContactPersonForChangeBtn) {
					this.readContactPersonForChangeBtn = true;
					this.readContactPersonData(hdrModel.System);
				}

				//each time call
				if (tabChangeMdl.getData().SelecteIconTab === "ID_OP_ACCOUNT_DATA" && !this.readAccountDataForChangeBtn) {
					this.readAccountDataForChangeBtn = true;
					if (hdrModel.CompnyCodeKey.toString() !== "") {
						this.onAccountingData(hdrModel.CompnyCodeKey, false);
					} else {
						this.onAccountingDataWithOutCompanyCode("");
					}
				}

				if (tabChangeMdl.getData().SelecteIconTab === "ID_OP_SALES_DATA" && !this.readSalesDataForChangeBtn) {
					this.readSalesDataForChangeBtn = true;
					this.readSalesData();
					this.readTaxClassification();
					this.createOtherPartnerFunctionModelSet(hdrModel.AccountGrpKey);
					this.createSoldtoPartnerFunctionModelSet(hdrModel.AccountGrpKey);
					this.readDefaultPartnerFunctions(hdrModel.AccountGrpKey);
					this.readPartnerFunction(hdrModel.AccountGrpKey);
				}

				if (tabChangeMdl.getData().SelecteIconTab === "ID_OP_SALES_DATA" && !this.readStatusDataForChangeBtn) {
					this.readStatusDataForChangeBtn = true;
					this.readStatusData(hdrModel.System);
				}*/

			//delete duplicate iban
			/*	var ibanList = this.getView().getModel("CustomerDataSet").getData().BankDetails.BankIBan;
				var arrayDupIban = [];
				for (var r = 0; r < ibanList.length; r++) {
					if (ibanList[r].IBan !== "" && ibanList[r].IBan !== undefined) {
						arrayDupIban.push(ibanList[r]);
					}
				}

				if (arrayDupIban.length === 0) {
					arrayDupIban.push({
						IBan: "",
						Country: "",
						BankKey: "",
				...
				for (var r = 0; r < arrayDupIban.length; r++) {
					arrayDupIban[r].TotalCount = (r + 1) + " of " + arrayDupIban.length;
					if (r === 0) {
						arrayDupIban[r].VisibleBasedOnNext = true;
					} else {
						arrayDupIban[r].VisibleBasedOnNext = false;
					}
				}
				this.getView().getModel("CustomerDataSet").getData().BankDetails.BankIBan = arrayDupIban;
		...
		bindTextNoteF4ValuesBasedOnFirstF4Classific: function() {
			...
			for (var i = 0; i < TextNoteTabData.length; i++) {
				...
				var ValueArr = [];
				for (var j = 0; j < f4Data.length; j++) {
					if (TextNoteTabData[i].ClassificationType.toString() !== "" && TextNoteTabData[i].ClassificationType.toString() === f4Data[j].ClassificationType
						.toString()) {
						classic = f4Data[j].NavClassification.results;
						if (classic.length === undefined) {
							classic = Array(classic);
						}
						ValueArr = f4Data[j].NavValue.results;
		...
		createCustomerDataTab: function(oData) {
			...
			for (var r = 0; r < bankDet.length; r++) {
				dataCust.BankDetails.BankIBan.push({
					...
					CountryDesc: bankDet[r].Countryname ? bankDet[r].Countryname.toString().trim() : "",
			...
			for (var i = 0; i < unlPints.length; i++) {
				...
				dataCust.UnloadingPoint.UnloadPointArr.push({
					...
					GoodsRecHrsDesc: unlPints[i].GoodsrecText ? unlPints[i].GoodsrecText.toString().trim() : "",
					Default: unlPints[i].Defab ? unlPints[i].Defab : false,
					CalenderKey: unlPints[i].CalendarKey ? unlPints[i].CalendarKey.toString().trim() : "",
					CalenderKeyDesc: unlPints[i].Factorycaltext ? unlPints[i].Factorycaltext.toString().trim() : "",
		...
		textNoteTabInitialModelCreate: function(Data) {
			...
			var oReslutsTextNotes = Data;
			var oResultsText = oReslutsTextNotes.N_TextNotes.results;
			var oResultsTextClassification = oReslutsTextNotes.N_Classif.results;
			for (var T = 0; T < oResultsText.length; T++) {
				data2.TextNotes.TextNotes2.push({
					Level: oResultsText[T].TextLevel ? oResultsText[T].TextLevel.toString().trim() : "",
					LevelDesc: oResultsText[T].TextLevel ? oResultsText[T].TextLevel.toString().trim() : "",
					Language: oResultsText[T].Language ? oResultsText[T].Language.toString().trim() : "",
					LanguageDesc: oResultsText[T].LanguageDesc ? oResultsText[T].LanguageDesc.toString().trim() : "",
					TextType: oResultsText[T].TextType ? oResultsText[T].TextType.toString().trim() : "",
					TextTypeDesc: oResultsText[T].TextType ? oResultsText[T].TextType.toString().trim() : "",
					FreeText: oResultsText[T].FirstLine ? oResultsText[T].FirstLine.toString().trim() : "",
					enable: (oResultsText[T].FirstLine.toString().trim().length > 40) ? true : false,
					LevelMessage: "",
					LevelState: "None",
					LanguageMessage: "",
					LanguageState: "None",
			...
			for (var T = 0; T < oResultsTextClassification.length; T++) {
				data2.Classification.Classification2.push({
					"ClassificationType": oResultsTextClassification[T].Klart ? oResultsTextClassification[T].Klart.toString().trim() : "",
					"ClassificationTypeDesc": oResultsTextClassification[T].ClassTypeDesc ? oResultsTextClassification[T].ClassTypeDesc.toString().trim() : "",
					"Classification": oResultsTextClassification[T].Class ? oResultsTextClassification[T].Class.toString().trim() : "",
					"ClassificationDesc": oResultsTextClassification[T].Class ? oResultsTextClassification[T].Class.toString().trim() : "",
					"Value": oResultsTextClassification[T].Atwrt ? oResultsTextClassification[T].Atwrt.toString().trim() : "",
					"ValueDesc": oResultsTextClassification[T].Atwrt ? oResultsTextClassification[T].Atwrt.toString().trim() : "",
			...
			if (data2.Classification.Classification2.length === 0) {
				data2.Classification.Classification2.push({
					"ClassificationType": "",
					"Classification": "",
					"Value": "",
			...
			if (data2.TextNotes.TextNotes2.length === 0) {
				this.onPressAddTextNotes();
			}
		},
		...
		createContactDetailsTab: function(oData) {
			...
			for (var i = 0; i < contPer.length; i++) {
				contactDetails.ContactDetails.ContactPerson.ContactPersonList.push({
					...
					"LanguageDesc": contPer[i].ParlaDesc ? contPer[i].ParlaDesc.toString().trim() : "",
					"Function": contPer[i].Pafkt ? contPer[i].Pafkt.toString().trim() : "",
					"FunctionDesc": contPer[i].ContactpersonFuncDesc ? contPer[i].ContactpersonFuncDesc.toString().trim() : "",
					"Department": contPer[i].Abtnr ? contPer[i].Abtnr.toString().trim() : "",
					"DepartmentDesc": contPer[i].ContactpersonDeptDesc ? contPer[i].ContactpersonDeptDesc.toString().trim() : "",
					...
					"MethodDesc": contPer[i].MethodDesc ? contPer[i].MethodDesc.toString().trim() : "",
		...
		onPressSubmitCustomerCreate: function(req) {
			var that = this;
			// var oDataRead = that.getView().getModel("DisplayReadCall").getData();
			//Resubmit & Review scenario
			var oDataRead = {};
			if (this.resubmitScenario === false && this.reviewScenario === false) {
				...
				var delInd = (modl !== undefined && modl.ArrayExtIdent !== undefined && modl.ArrayExtIdent.delInd !== undefined) ? modl.ArrayExtIdent
					.delInd : "N";
				oDataRead = that.getView().getModel("DisplayReadCall").getData();
			} else {
				delInd = "";
				var oModelReq = new sap.ui.model.json.JSONModel({});
				that.getView().setModel(oModelReq, "DisplayReadCall");
				oDataRead = that.getView().getModel("DisplayReadCall").getData();
			}
			var payload = {
				BusinessPartnerData: {},
			...
			payload.BusinessPartnerData.CustomerBusinessArea.roleDescription = (req === "Old" && modl !== undefined && modl.ArrayExtIdent !==
					undefined && modl.ArrayExtIdent.applnRoleRowid !== undefined && modl.ArrayExtIdent.applnRoleRowid.roleDescription !== undefined) ?
				modl.ArrayExtIdent.applnRoleRowid.roleDescription : "";
			...
			for (var d = 0; d < contPerson.length; d++) {
				if (contPerson[d].LastName.toString().trim() !== "") {
					payload.ContactDetails.ContactPersons.push({
						...
						"ContactpersonLangDesc": (req === "Old" && contPerson[d].LanguageDesc) ? contPerson[d].LanguageDesc : "",
						"languageRowid": "",
						...
						"ContactpersonFuncDesc": (req === "Old" && contPerson[d].FunctionDesc) ? contPerson[d].FunctionDesc : "",
						"Department": contPerson[d].Department.toString().trim(),
						"ContactpersonDeptDesc": (req === "Old" && contPerson[d].DepartmentDesc) ? contPerson[d].DepartmentDesc : "",
						...
						"MethodDesc": (req === "Old" && contPerson[d].MethodDesc) ? contPerson[d].MethodDesc : "",
			...
			for (var i = 0; i < unlodPointTab.length; i++) {
				...
				if (unlodPointTab[i].UnloadPoint.toString().trim() !== "" && unlodPointTab[i].CalenderKey.toString().trim() !== "") {
					payload.CustomerData.UnloadingPoints.push({
						...
						"GoodsrecText": (req === "Old" && unlodPointTab[i].GoodsRecHrsDesc) ? unlodPointTab[i].GoodsRecHrsDesc : "",
						"Default": unlodPointTab[i].Default,
						"CalendarKey": unlodPointTab[i].CalenderKey.toString().trim(),
						"CalendarKeyDesc": (req === "Old" && unlodPointTab[i].CalenderKeyDesc) ? unlodPointTab[i].CalenderKeyDesc : "",
			...
			for (var r = 0; r < bankItems.length; r++) {
				...
				if (bankItems[r].Country.toString().trim() !== "" && bankItems[r].BankKey.toString().trim() !== "" && bankItems[r].BankAccount.toString()
					...
					payload.CustomerData.BankData.BankDetails.push({
						...
						"Countryname": (req === "Old" && bankItems[r].CountryDesc) ? bankItems[r].CountryDesc : "",
			...
			//description
			payload.CustomerData.GeneralData.CommLangDesc = (req === "Old" && oDataRead.CommLangDesc) ? oDataRead.CommLangDesc : "";
			payload.CustomerData.GeneralData.TransportationZoneDesc = (req === "Old" && oDataRead.TransportationZoneDesc) ? oDataRead.TransportationZoneDesc :
				"";
			payload.CustomerData.GeneralData.AltTransportationZoneDesc = (req === "Old" && oDataRead.AltTransportationZoneDesc) ? oDataRead.AltTransportationZoneDesc :
				"";
			payload.CustomerData.GeneralData.GroupKeyDesc = (req === "Old" && oDataRead.GroupKeyDesc) ? oDataRead.GroupKeyDesc : "";
			payload.CustomerData.GeneralData.Reg_Struct_GroupDesc = (req === "Old" && oDataRead.RegStrGrpDesc) ? oDataRead.RegStrGrpDesc : "";
			...
			for (var m = 0; m < TextNote.length; m++) {
				if (TextNote[m].Level.toString().trim() !== "" && TextNote[m].TextType.toString().trim() !== "") {
					payload.TextNotes.Text.push({
						...
						"LanguageDesc": (req === "Old" && TextNote[m].LanguageDesc) ? TextNote[m].LanguageDesc : "",
			...
			for (var l = 0; l < classTextNote.length; l++) {
				if (classTextNote[l].ClassificationType.toString().trim() !== "" && classTextNote[l].Classification.toString().trim() !== "" &&
					...
					payload.TextNotes.TextClassification.push({
						...
						"ClassTypeDesc": (req === "Old" && classTextNote[l].ClassificationTypeDesc) ? classTextNote[l].ClassificationTypeDesc : "",
			...
			if (payload.BusinessPartnerData.CustomerBusinessArea.CompanyCode !== "") {
				...
				//description
				payload.AccountingData.AccountManagement.ReconciliationAccntDesc = (req === "Old" && oDataRead.ReconciliationAccntDesc) ?
					oDataRead.ReconciliationAccntDesc : "";
				payload.AccountingData.AccountManagement.SortKeyDesc = (req === "Old" && oDataRead.SortKeyDesc) ? oDataRead.SortKeyDesc : "";
				...
				//description
				payload.AccountingData.PaymentData.TermsOfPaymentDesc = (req === "Old" && oDataRead.TermsOfPaymentDesc) ? oDataRead.TermsOfPaymentDesc :
					"";
				payload.AccountingData.PaymentData.CreditMemoPaymntTrmsDesc = (req === "Old" && oDataRead.CreditMemoPaymntTrmsDesc) ? oDataRead.CreditMemoPaymntTrmsDesc :
					"";
				payload.AccountingData.PaymentData.BillExchPaymntTermsDesc = (req === "Old" && oDataRead.BillExchPaymntTermsDesc) ? oDataRead.BillExchPaymntTermsDesc :
					"";
				payload.AccountingData.PaymentData.ToleranceGroupDesc = (req === "Old" && oDataRead.ToleranceGroupDesc) ? oDataRead.ToleranceGroupDesc :
					"";
				payload.AccountingData.PaymentData.PaymentMethodsDesc = (req === "Old" && oDataRead.PaymentMethodsDesc) ? oDataRead.PaymentMethodsDesc :
					"";
				payload.AccountingData.PaymentData.HouseBankDesc = (req === "Old" && oDataRead.HouseBankDesc) ? oDataRead.HouseBankDesc : "";
				payload.AccountingData.PaymentData.SelectionRuleDesc = (req === "Old" && oDataRead.SelectionRuleDesc) ? oDataRead.SelectionRuleDesc :
					"";
				...
				//DESCRPTION
				payload.AccountingData.Correspondence.DunngProcedureDesc = (req === "Old" && oDataRead.DunngProcedureDesc) ? oDataRead.DunngProcedureDesc :
					"";
				payload.AccountingData.Correspondence.DunningClerkDesc = (req === "Old" && oDataRead.DunningClerkDesc) ? oDataRead.DunningClerkDesc :
					"";
				payload.AccountingData.Correspondence.DunningBlockDesc = (req === "Old" && oDataRead.DunningBlockDesc) ? oDataRead.DunningBlockDesc :
					"";
				payload.AccountingData.Correspondence.DunningAreaDesc = (req === "Old" && oDataRead.DunningAreaDesc) ? oDataRead.DunningAreaDesc :
					"";
				payload.AccountingData.Correspondence.ClerkAbbrvDesc = (req === "Old" && oDataRead.ClerkAbbrvDesc) ? oDataRead.ClerkAbbrvDesc : "";
				payload.AccountingData.Correspondence.AccntStatemnt = (req === "Old" && oDataRead.AccntStatemntDesc) ? oDataRead.AccntStatemntDesc :
					"";
			}
			//status data
			...
			//description
			payload.StatusData.BlockData.OrderBlockDesc = (req === "Old" && oDataRead.OrderBlockDesc) ? oDataRead.OrderBlockDesc : "";
			payload.StatusData.BlockData.DeliveryBlockDesc = (req === "Old" && oDataRead.DeliveryBlockDesc) ? oDataRead.DeliveryBlockDesc : "";
			payload.StatusData.BlockData.BillingBlockDesc = (req === "Old" && oDataRead.BillingBlockDesc) ? oDataRead.BillingBlockDesc : "";
			...
			if (payload.BusinessPartnerData.CustomerBusinessArea.SalesOrg !== "" && payload.BusinessPartnerData.CustomerBusinessArea.DistributionChannel !==
				...
				//description
				payload.SalesData.SalesOrder.SalesDistrictDesc = (req === "Old" && oDataRead.SalesDistrictDesc) ? oDataRead.SalesDistrictDesc : "";
				payload.SalesData.SalesOrder.SalesOfficeDesc = (req === "Old" && oDataRead.SalesOfficeDesc) ? oDataRead.SalesOfficeDesc : "";
				payload.SalesData.SalesOrder.SalesGroupDesc = (req === "Old" && oDataRead.SalesGroupDesc) ? oDataRead.SalesGroupDesc : "";
				payload.SalesData.SalesOrder.CustomerGroupDesc = (req === "Old" && oDataRead.CustomerGroupDesc) ? oDataRead.CustomerGroupDesc : "";
				payload.SalesData.SalesOrder.CurrencyDesc = (req === "Old" && oDataRead.CurrencyDesc) ? oDataRead.CurrencyDesc : "";
				payload.SalesData.SalesOrder.ExchRateTypeDesc = (req === "Old" && oDataRead.ExchRateTypeDesc) ? oDataRead.ExchRateTypeDesc : "";
				payload.SalesData.SalesOrder.PriceGrpDesc = (req === "Old" && oDataRead.PriceGrpDesc) ? oDataRead.PriceGrpDesc : "";
				payload.SalesData.SalesOrder.CustPricProcDesc = (req === "Old" && oDataRead.CustPricProcDesc) ? oDataRead.CustPricProcDesc : "";
				payload.SalesData.SalesOrder.PriceListDesc = (req === "Old" && oDataRead.PriceListDesc) ? oDataRead.PriceListDesc : "";
				payload.SalesData.SalesOrder.CustStatsGrpDesc = (req === "Old" && oDataRead.CustStatsGrpDesc) ? oDataRead.CustStatsGrpDesc : "";
				...
				//description
				payload.SalesData.Shipping.DeliveryPriorityDesc = (req === "Old" && oDataRead.DeliveryPriorityDesc) ? oDataRead.DeliveryPriorityDesc :
					"";
				payload.SalesData.Shipping.ShippingCondDesc = (req === "Old" && oDataRead.ShippingCondDesc) ? oDataRead.ShippingCondDesc : "";
				payload.SalesData.Shipping.DeliveringPlantDesc = (req === "Old" && oDataRead.DeliveringPlantDesc) ? oDataRead.DeliveringPlantDesc :
					"";
				payload.SalesData.Shipping.PartialDelvryPerItemDesc = (req === "Old" && oDataRead.PartialDelvryPerItemDesc) ? oDataRead.PartialDelvryPerItemDesc :
					"";
				...
				payload.SalesData.BillingDocument.InvoiceDatesDesc = (req === "Old" && oDataRead.InvoiceDatesDesc) ? oDataRead.InvoiceDatesDesc :
					"";
				payload.SalesData.BillingDocument.InvoiceListDatesDesc = (req === "Old" && oDataRead.InvoiceListDatesDesc) ? oDataRead.InvoiceListDatesDesc :
					"";
				payload.SalesData.BillingDocument.Incoterms1Desc = (req === "Old" && oDataRead.Incoterms1Desc) ? oDataRead.Incoterms1Desc :
					"";
				payload.SalesData.BillingDocument.SalestermsOfPaymentDesc = (req === "Old" && oDataRead.SalestermsOfPaymentDesc) ? oDataRead.SalestermsOfPaymentDesc :
					"";
				payload.SalesData.BillingDocument.AcctAssgmtGroupDesc = (req === "Old" && oDataRead.AcctAssgmtGroupDesc) ? oDataRead.AcctAssgmtGroupDesc :
					"";
				...
				//description
				payload.SalesData.AdditionalFields.CustomerGroup1Desc = (req === "Old" && oDataRead.CustomerGroup1Desc) ? oDataRead.CustomerGroup1Desc :
					"";
				payload.SalesData.AdditionalFields.CustomerGroup2Desc = (req === "Old" && oDataRead.CustomerGroup2Desc) ? oDataRead.CustomerGroup2Desc :
					"";
				payload.SalesData.AdditionalFields.CustomerGroup3Desc = (req === "Old" && oDataRead.CustomerGroup3Desc) ? oDataRead.CustomerGroup3Desc :
					"";
				payload.SalesData.AdditionalFields.CustomerGroup4Desc = (req === "Old" && oDataRead.CustomerGroup4Desc) ? oDataRead.CustomerGroup4Desc :
					"";
				payload.SalesData.AdditionalFields.CustomerGroup5Desc = (req === "Old" && oDataRead.CustomerGroup5Desc) ? oDataRead.CustomerGroup5Desc :
					"";
				payload.SalesData.AdditionalFields.CustomerGroup6Desc = (req === "Old" && oDataRead.CustomerGroup6Desc) ? oDataRead.CustomerGroup6Desc :
					"";
			...
			if (payload.BusinessPartnerData.CustomerBusinessArea.SalesOrg !== "" && payload.BusinessPartnerData.CustomerBusinessArea.DistributionChannel !==
				...
				for (var lb = 0; lb < taxClass.length; lb++) {
					payload.SalesData.BillingDocument.TaxClassifications.push({
						...
						"TaxClassificationDescription": (req === "Old" && taxClass[lb].TaxclassificationDesc) ? taxClass[lb].TaxclassificationDesc : ""
					});
				}
			}
				...
				for (var z = 0; z < defPartFun.length; z++) {
					if (defPartFun[z].PartnerFunction.toString().trim() !== "" && defPartFun[z].Number.toString().trim() !== "") {
						payload.SalesData.PartnerFunctions.push({
							...
							"PartnerFuncDesc": (req === "Old" && defPartFun[z].PartnerName) ? defPartFun[z].PartnerName : ""
						});
					}
				}
				...
				for (var z = 0; z < otherPar.length; z++) {
					if (otherPar[z].PartnerFunction.toString().trim() !== "" && otherPar[z].Number.toString().trim() !== "") {
						payload.SalesData.PartnerFunctions.push({
							...
							"PartnerFuncDesc": (req === "Old" && otherPar[z].PartnerName) ? otherPar[z].PartnerName : ""
				...
				for (var z = 0; z < toBeLink.length; z++) {
					if (toBeLink[z].PartnerFunction.toString().trim() !== "" && toBeLink[z].Number.toString().trim() !== "") {
						payload.SalesData.PartnerFunctions.push({
							...
							"PartnerFuncDesc": (req === "Old" && toBeLink[z].PartnerName) ? toBeLink[z].PartnerName : ""
						});
					}
				}
			}
			...
			if (this.resubmitScenario === true) {
				var kunnr = this.InputData.d.startTypeINPUT.start.DOCustomerChange.WorkFlowDetails.KUNNR;
			} else {
		...
		submitFinal: function(processId) {
			...
			//Resubmit & Review scenario
			if (this.resubmitScenario === false) {
				//old row id need to add
				var busPart = sap.ui.getCore().getModel("ExtBusPartnerModelSet").getData().results.BUSINESSPARTNER;
				var dataPassed = sap.ui.getCore().getModel("searchExtSubTableModelSet").getData();
				that.payloadStoreOldDataBeforeChangeTemp.BusinessPartnerData.CustomerBusinessArea.itoLogicalCompRowid = dataPassed.ArrayExtIdent.itoLogicalCompRowid !==
					undefined ? dataPassed.ArrayExtIdent.itoLogicalCompRowid.rowidObject.toString() : "";
				that.payloadStoreOldDataBeforeChangeTemp.BusinessPartnerData.CustomerBusinessArea.applnRoleRowid = dataPassed.ArrayExtIdent.applnRoleRowid !==
					undefined ? dataPassed.ArrayExtIdent.applnRoleRowid.rowidObject.toString() : "";
				that.payloadStoreOldDataBeforeChangeTemp.CustomerData.GeneralData.transZoneCodeRowid = dataPassed.ArrayExtIdent.transZoneCodeRowid !==
					undefined ? dataPassed.ArrayExtIdent.transZoneCodeRowid.rowidObject.toString() : "";
				that.payloadStoreOldDataBeforeChangeTemp.BusinessPartnerData.CustomerBusinessArea.companyCodeRowid = (dataPassed.ArrayExtIdent.CUSCOMPANYCODE !==
					undefined && dataPassed.ArrayExtIdent.CUSCOMPANYCODE.item !== undefined && dataPassed.ArrayExtIdent.CUSCOMPANYCODE.item.companyCodeRowid !==
					undefined) ? dataPassed.ArrayExtIdent.CUSCOMPANYCODE.item.companyCodeRowid.rowidObject.toString() : "";
				that.payloadStoreOldDataBeforeChangeTemp.BusinessPartnerData.CustomerBusinessArea.salesOrganizationRowid = (dataPassed.ArrayExtIdent
						.CUSSALESAREA !== undefined && dataPassed.ArrayExtIdent.CUSSALESAREA.item !== undefined && dataPassed.ArrayExtIdent.CUSSALESAREA
						.item
						.salesOrganizationRowid !== undefined) ? dataPassed.ArrayExtIdent.CUSSALESAREA.item.salesOrganizationRowid.rowidObject.toString() :
					"";
				that.payloadStoreOldDataBeforeChangeTemp.BusinessPartnerData.CustomerBusinessArea.distributionChannelRowid = (dataPassed.ArrayExtIdent
						.CUSSALESAREA !== undefined && dataPassed.ArrayExtIdent.CUSSALESAREA.item !== undefined && dataPassed.ArrayExtIdent.CUSSALESAREA
						.item
						.distributionChannelRowid !== undefined) ? dataPassed.ArrayExtIdent.CUSSALESAREA.item.distributionChannelRowid.rowidObject.toString() :
					"";
				that.payloadStoreOldDataBeforeChangeTemp.BusinessPartnerData.CustomerBusinessArea.divisionRowid = (dataPassed.ArrayExtIdent.CUSSALESAREA !==
					undefined && dataPassed.ArrayExtIdent.CUSSALESAREA.item !== undefined && dataPassed.ArrayExtIdent.CUSSALESAREA.item.divisionRowid !==
					undefined) ? dataPassed.ArrayExtIdent.CUSSALESAREA.item.divisionRowid.rowidObject.toString() : "";
				that.payloadStoreOldDataBeforeChangeTemp.BusinessPartnerData.TaxData.taxNumberTypeRowid = busPart.taxNumberTypeRowid !== undefined ?
					busPart.taxNumberTypeRowid.rowidObject.toString() : "";
				that.payloadStoreOldDataBeforeChangeTemp.BusinessPartnerData.TaxData.taxTypeRowid = busPart.taxTypeRowid !== undefined ? busPart.taxTypeRowid
					.rowidObject.toString() : "";
				...
				that.payloadStoreOldDataBeforeChangeTemp.BusinessPartnerData.CustomerBusinessArea.BProwid = busPart.rowidObject ? busPart.rowidObject
					.toString() :
					"";
				that.payloadStoreOldDataBeforeChangeTemp.BusinessPartnerData.CustomerBusinessArea.CCrowid = (dataPassed.ArrayExtIdent.CUSCOMPANYCODE !==
						undefined && dataPassed.ArrayExtIdent.CUSCOMPANYCODE.item !== undefined) ? dataPassed.ArrayExtIdent.CUSCOMPANYCODE.item.rowidObject
					.toString() :
					"";
				that.payloadStoreOldDataBeforeChangeTemp.BusinessPartnerData.CustomerBusinessArea.SArowid = (dataPassed.ArrayExtIdent
						.CUSSALESAREA !== undefined && dataPassed.ArrayExtIdent.CUSSALESAREA.item !== undefined) ? dataPassed.ArrayExtIdent.CUSSALESAREA
					.item.rowidObject.toString() : "";
				that.payloadStoreOldDataBeforeChangeTemp.BusinessPartnerData.CustomerBusinessArea.CUSINTrowid = dataPassed.ArrayExtIdent ?
					dataPassed.ArrayExtIdent.rowidObject.toString() : "";
		...
		createAccountyEmptyModel: function(oData) {
			var accData = {
				...
				Correspondence: {
					...
					LastDunned: Formatter.readLastDunnedDate(oData.LastDunned),
					//LastDunned: oData.LastDunned ? oData.LastDunned.toString().trim() : "",
					...
					success: function(oData, oResponse) {
						...
						if (oData.results[0].Response.includes("<h1>Error</h1>")) {
							...
							for (var i = 0; i < oODataJSONModel.getData().results.d.results.length; i++) {
								oODataJSONModel.getData().results.d.results[i].Taxclassification = "";
							}
							that.getView().setModel(oODataJSONModel, "TaxClassificationComboSet");
							//Resubmit & Review scenario
		...
		createStatusDetailstab: function(oData) {
			var statusDetails = {
				...
				BlockData: {
					PostingBlock: oData.PostingBlock ? true : false,
					...
					BlockSalesReport: oData.BlockSalesSupport ? true : false,
		...
		readDefaultPartnerFuncRedDisplay: function(OData) {
			var partData = OData.N_PartnerFnct.results;
			var otherPart = [];
			for (var i = 0; i < partData.length; i++) {
				...
				otherPart.push({
					...
					Editable: partData[i].PartnerFuncDesc === "Sold-to party" ? false : true,
					custNumberValueState: "None",
					custNumberValueStateText: ""
		...
		createSalesDataTabFirstReadCall: function(oData) {
			var salesData = {
				SalesOrder: {
					...
					AcctAtCust: oData.SalesAcctAtCust ? oData.SalesAcctAtCust.toString().trim() : "",
				...
				Shipping: {
					...
					// PartialDeliveryPerItem: oData.PartialDelvryPerItem ? oData.PartialDelvryPerItem.toString() !== "" ? oData.PartialDelvryPerItem.toString() : " " : "",
		...
		//read from display
		createOtherPartnerFunctionModelSetDisplay: function(accountGroup, OData) {
			var partnerFunction = {
				OtherPartnerFunctions: [{
					PartnerFunction: "",
					PartnerName: "",
		...
		//display only
		createSoldtoPartnerFunctionModelSetDisplayRead: function(accountGroup, OData) {
			var SoldtoPartnerFunction = {
				SoldtoPartnerFunctions: [{
					PartnerFunction: "",
					PartnerName: "",
		...
		onClickEditRecod: function() {
			...
			var that = this;
			var selObj = this.getView().getModel("HeaderModelSet").getData();
			var UserCode1 = sap.ui.getCore().getModel("UserInfoSet").getData().UserCode1;
			var paylod = {
				"UserID": UserCode1,
				"CompanyCode": selObj.CompnyCodeKey.toString().trim(),
				"SalesOrganization": selObj.SalesOrgKey.toString().trim(),
				"Division": selObj.DivisionKey.toString().trim(),
				"DistributionChannel": selObj.DistribChnnlKey.toString().trim(),
				"System": selObj.System.toString().trim(),
				"AccountGroup": selObj.AccountGrpKey.toString().trim()
			};

			if (selObj.System.toString().trim() != "") {
				var BusinessPartnerData = {
					"URL": "/RESTAdapter/ChangeCust_UserAuth/LoopBack",
					"Request": JSON.stringify(paylod)
				};
				var oBusyDialog = new sap.m.BusyDialog();
				oBusyDialog.open();
				this.oModel.create("/BusinessPartnerSet", BusinessPartnerData, {
					success: function(response) {
						...
						if (response.Response.includes("<h1>Error</h1>")) {
							...
							var dataOutPut = JSON.parse(response.Response);
							if (dataOutPut.MESSAGE === "AUTHORIZED") {
								that.readCallForPendingRequist();
							} else {
								that.errMsg(that.getView().getModel("i18n").getProperty("notAuthorizedForThisUserChange"));
							}
						}
				...
				"/BusinessPartnerSet", {
					...
					success: function(oData, oResponse) {
						...
						if (oData.results[0].Response.includes("<h1>Error</h1>")) {
							...
							var oModelReq = new sap.ui.model.json.JSONModel(oSearch.d.results[0]);
							that.getView().setModel(oModelReq, "DisplayReadCall");
							//read header data
							...
							that.readSalesData();
							that.readSalesGroup(oSearch.d.results[0].SalesOffice);
							that.readTaxClassification();
							that.createOtherPartnerFunctionModelSetDisplay(hdrModel.AccountGrpKey, oSearch.d.results[0]);
							that.createSoldtoPartnerFunctionModelSetDisplayRead(hdrModel.AccountGrpKey, oSearch.d.results[0]);
							that.readDefaultPartnerFuncRedDisplay(oSearch.d.results[0]);
							...
							that.onAccountingDataPaymentMethod(hdrModel.CompnyCodeKey, hdrModel.System);

							that.readStatusData(hdrModel.System);
							if (hdrModel.CompnyCodeKey.toString() !== "") {
								that.onAccountingData(hdrModel.CompnyCodeKey, false, hdrModel.System);
							} else {
		...
		readInitialDisplayOnly: function(finalstruct) {
			var accGrp = {
				...
				CompanyCodeCond: "HIDE",
				SalesOrgCond: "HIDE",
				DistriChanlCond: "HIDE",
				DivisionCond: "HIDE",
				SystemCond: "HIDE",
				AccountGrpCond: "HIDE",
				ApplicIdCond: "HIDE",
		...
		checktaskStatus: function() {
			...
			if (this.taskStatus === "READY") {
				var oModel = new sap.ui.model.json.JSONModel({
					...
					"AccountGroupCB": false, //this.resubmitScenario === true ? true : false,
					"AccountGroupInput": true, //this.reviewScenario === true ? true : false
		...
		bindChangeCustomerModel: function() {
			...
			if (this.resubmitScenario === true) {
				this.basedOnAccountGroupEnableMandatoryOptional(AccountGrpKey);
			} else if (this.reviewScenario === true) {
				this.getView().getModel("RequiredOptionalSet").getData().UploadDoc = true;
		...
		bindCustomerBusinessAreaModel: function() {
			...
			var hdr = {
				CompnyCodeKey: CustomerBusinessAreaNew.CompanyCode !== "NA" ? CustomerBusinessAreaNew.CompanyCode : "",
		...
		bindAccountManagement: function() {
			...
			AccountManagement.ReconAccountTooltip = Formatter.createTooltipConcat(this.reviewScenario, AccountManagementDataOld.Recon_Account,
				AccountManagementDataNew.Recon_Account,
				AccountManagementDataOld.ReconciliationAccntDesc);
			AccountManagement.HeadOfficeTooltip = Formatter.createTooltipText(this.reviewScenario, AccountManagementDataOld.Head_Office,
				AccountManagementDataNew.Head_Office,
				AccountManagementDataOld.Head_Office);
			AccountManagement.SortKeyTooltip = Formatter.createTooltipConcat(this.reviewScenario, AccountManagementDataOld.Sort_Key,
				AccountManagementDataNew.Sort_Key,
				AccountManagementDataOld.SortKeyDesc);
			AccountManagement.PrevAcctNoTooltip = Formatter.createTooltipText(this.reviewScenario, AccountManagementDataOld.Prev_Acct_No,
				AccountManagementDataNew.Prev_Acct_No,
				AccountManagementDataOld.Prev_Acct_No);
			AccountManagement.BuyingGroupTooltip = Formatter.createTooltipText(this.reviewScenario, AccountManagementDataOld.Buying_Group,
				AccountManagementDataNew.Buying_Group,
				AccountManagementDataOld.Buying_Group);
		...
		bindPaymentData: function() {
			...
			PaymentTerms.terms_of_paymentState = this.reviewScenario !== true ? "None" : PaymentDataNew.Terms_Of_Payment !== PaymentDataOld.Terms_Of_Payment ?
				"Warning" : "None";
			PaymentTerms.TermsofPaymentTooltip = Formatter.createTooltipConcat(this.reviewScenario, PaymentDataOld.Terms_Of_Payment,
				PaymentDataNew.Terms_Of_Payment, PaymentDataOld.TermsOfPaymentDesc);
			...
			PaymentDataModel.TermsofPaymentTooltip = Formatter.createTooltipConcat(this.reviewScenario, PaymentDataOld.Terms_Of_Payment,
				PaymentDataNew.Terms_Of_Payment,
				PaymentDataOld.TermsOfPaymentDesc);
			PaymentDataModel.CreditMemoPaytTermTooltip = Formatter.createTooltipConcat(this.reviewScenario, PaymentDataOld.Credit_Memo_Payment_Term,
				PaymentDataNew.Credit_Memo_Payment_Term,
				PaymentDataOld.CreditMemoPaymntTrmsDesc);
			PaymentDataModel.BeChangesPayTermTooltip = Formatter.createTooltipConcat(this.reviewScenario, PaymentDataOld.B_E_Changes_Payment_Term,
				PaymentDataNew.B_E_Changes_Payment_Term,
				PaymentDataOld.BillExchPaymntTermsDesc);
			PaymentDataModel.ToleranceGroupTooltip = Formatter.createTooltipConcat(this.reviewScenario, PaymentDataOld.Tolerance_Group,
				PaymentDataNew.Tolerance_Group,
				PaymentDataOld.ToleranceGroupDesc);
			PaymentDataModel.PaymentMethodsTooltip = Formatter.createTooltipConcat(this.reviewScenario, PaymentDataOld.Payment_Methods,
				PaymentDataNew.Payment_Methods,
				PaymentDataOld.PaymentMethodsDesc);
			PaymentDataModel.HouseBankTooltip = Formatter.createTooltipConcat(this.reviewScenario, PaymentDataOld.House_Bank, PaymentDataNew.House_Bank,
				PaymentDataOld.HouseBankDesc);
			PaymentDataModel.SelectionRuleTooltip = Formatter.createTooltipConcat(this.reviewScenario, PaymentDataOld.Selection_Rule,
				PaymentDataNew.Selection_Rule,
				PaymentDataOld.SelectionRuleDesc);
		...
		bindCorrespondence: function() {
			...
			CorrespondenceData.LastDunned = Formatter.readLastDunnedDateApproval(CorrespondenceNew.Last_Dunned);
			...
			CorrespondenceData.DunnProcedureTooltip = Formatter.createTooltipConcat(this.reviewScenario, CorrespondenceOld.Dunn_Procedure,
				CorrespondenceNew.Dunn_Procedure,
				CorrespondenceOld.DunngProcedureDesc);
			CorrespondenceData.LastDunnedTooltip = Formatter.createTooltipText(this.reviewScenario, CorrespondenceOld.Last_Dunned,
				CorrespondenceNew.Last_Dunned,
				CorrespondenceOld.Last_Dunned);
			CorrespondenceData.DunningClerkTooltip = Formatter.createTooltipConcat(this.reviewScenario, CorrespondenceOld.Dunn_Procedure,
				CorrespondenceNew.Dunn_Procedure,
				CorrespondenceOld.DunngProcedureDesc);
			CorrespondenceData.DunningBlockTooltip = Formatter.createTooltipConcat(this.reviewScenario, CorrespondenceOld.Dunning_Block,
				CorrespondenceNew.Dunning_Block,
				CorrespondenceOld.DunningBlockDesc);
			CorrespondenceData.DunningLevelTooltip = Formatter.createTooltipText(this.reviewScenario, CorrespondenceOld.Dunning_Level,
				CorrespondenceNew.Dunning_Level,
				CorrespondenceOld.Dunning_Level);
			CorrespondenceData.DunningAreaTooltip = Formatter.createTooltipText(this.reviewScenario, CorrespondenceOld.Dunning_Area,
				CorrespondenceNew.Dunning_Area,
				CorrespondenceOld.Dunning_Area);
			CorrespondenceData.ClerkAbbrevTooltip = Formatter.createTooltipConcat(this.reviewScenario, CorrespondenceOld.Clerk_Abbrev,
				CorrespondenceNew.Clerk_Abbrev,
				CorrespondenceOld.ClerkAbbrvDesc);
			CorrespondenceData.AcctAtCustTooltip = Formatter.createTooltipText(this.reviewScenario, CorrespondenceOld.Acct_At_Cust,
				CorrespondenceNew.Acct_At_Cust,
				CorrespondenceOld.Acct_At_Cust);
			CorrespondenceData.CustomerUserTooltip = Formatter.createTooltipText(this.reviewScenario, CorrespondenceOld.Customer_User,
				CorrespondenceNew.Customer_User,
				CorrespondenceOld.Customer_User);
			CorrespondenceData.ClerksInternetTooltip = Formatter.createTooltipText(this.reviewScenario, CorrespondenceOld.Clerks_Internet,
				CorrespondenceNew.Clerks_Internet,
				CorrespondenceOld.Clerks_Internet);
			CorrespondenceData.AccountStatementTooltip = Formatter.createTooltipConcat(this.reviewScenario, CorrespondenceOld.Account_Statement,
				CorrespondenceNew.Account_Statement,
				CorrespondenceOld.AccntStatemnt);
		...
		bindSalesOrder: function() {
			...
			SalesOrder.CustStatGrpState = this.reviewScenario !== true ? "None" : SalesOrderDataNew.CustStatsGrp !== SalesOrderDataOld.CustStatsGrp ?
				"Warning" : "None";
			//////////////////ToolTip/////////////
			SalesOrder.SalesDistrictTooltip = Formatter.createTooltipConcat(this.reviewScenario, SalesOrderDataOld.SalesDistrict,
				SalesOrderDataNew.SalesDistrict,
				SalesOrderDataOld.SalesDistrictDesc);
			SalesOrder.SalesOfficeTooltip = Formatter.createTooltipConcat(this.reviewScenario, SalesOrderDataOld.SalesOffice, SalesOrderDataNew
				.SalesOffice,
				SalesOrderDataOld.SalesOfficeDesc);
			SalesOrder.SalesGroupTooltip = Formatter.createTooltipConcat(this.reviewScenario, SalesOrderDataOld.SalesGroup, SalesOrderDataNew.SalesGroup,
				SalesOrderDataOld.SalesGroupDesc);
			SalesOrder.CustomerGroupTooltip = Formatter.createTooltipConcat(this.reviewScenario, SalesOrderDataOld.CustomerGroup,
				SalesOrderDataNew.CustomerGroup,
				SalesOrderDataOld.CustomerGroupDesc);
			SalesOrder.CurrencyTooltip = Formatter.createTooltipConcat(this.reviewScenario, SalesOrderDataOld.Currency, SalesOrderDataNew.Currency,
				SalesOrderDataOld.CurrencyDesc);
			SalesOrder.AcctAtCustTooltip = Formatter.createTooltipText(this.reviewScenario, SalesOrderDataOld.AcctAtCust, SalesOrderDataNew.AcctAtCust,
				SalesOrderDataOld.AcctAtCust);
			SalesOrder.ExchangeRateTypeTooltip = Formatter.createTooltipConcat(this.reviewScenario, SalesOrderDataOld.ExchangeRateType,
				SalesOrderDataNew.ExchangeRateType,
				SalesOrderDataOld.ExchRateTypeDesc);
			SalesOrder.PriceGroupTooltip = Formatter.createTooltipConcat(this.reviewScenario, SalesOrderDataOld.PriceGroup, SalesOrderDataNew.PriceGroup,
				SalesOrderDataOld.PriceGrpDesc);
			SalesOrder.CustPricProcTooltip = Formatter.createTooltipConcat(this.reviewScenario, SalesOrderDataOld.CustPricProc,
				SalesOrderDataNew.CustPricProc,
				SalesOrderDataOld.CustPricProcDesc);
			SalesOrder.PriceListTooltip = Formatter.createTooltipConcat(this.reviewScenario, SalesOrderDataOld.PriceList, SalesOrderDataNew.PriceList,
				SalesOrderDataOld.PriceListDesc);
			SalesOrder.CustStatGrpTooltip = Formatter.createTooltipConcat(this.reviewScenario, SalesOrderDataOld.CustStatsGrp,
				SalesOrderDataNew
				.CustStatsGrp, SalesOrderDataOld.CustStatsGrpDesc);
		...
		bindShipping: function() {
			...
			///////////////ToolTip//////////
			Shipping.DeliveryPriorityTooltip = Formatter.createTooltipConcat(this.reviewScenario, ShippingDataOld.DeliveryPriority,
				ShippingDataNew.DeliveryPriority,
				ShippingDataOld.DeliveryPriorityDesc);
			Shipping.ShippingConditionsTooltip = Formatter.createTooltipConcat(this.reviewScenario, ShippingDataOld.ShippingConditions,
				ShippingDataNew.ShippingConditions,
				ShippingDataOld.ShippingCondDesc);
			Shipping.DeliveryPlantTooltip = Formatter.createTooltipConcat(this.reviewScenario, ShippingDataOld.DeliveringPlant, ShippingDataNew
				.DeliveringPlant,
				ShippingDataOld.DeliveringPlantDesc);
			Shipping.ProductionTimeFrameTooltip = Formatter.createTooltipText(this.reviewScenario, ShippingDataOld.PodTimeframe,
				ShippingDataNew.PodTimeframe,
				ShippingDataOld.PodTimeframe);
			Shipping.PartialDeliveryPerItemTooltip = Formatter.createTooltipConcat(this.reviewScenario, ShippingDataOld.PartialDeliveryPerItem,
				ShippingDataNew.PartialDeliveryPerItem,
				ShippingDataOld.PartialDelvryPerItemDesc);
			Shipping.MaxPartialDeliveriesTooltip = Formatter.createTooltipText(this.reviewScenario, ShippingDataOld.MaxPartialDeliveries,
				ShippingDataNew.MaxPartialDeliveries,
				ShippingDataOld.MaxPartialDeliveries);
		},
		...
		bindBillingDocument: function() {
			...
			/////////////////ToolTip////////////////////////
			BillingDocument.InvoiceDatesTooltip = Formatter.createTooltipConcat(this.reviewScenario, BillingDocumentDataOld.InvoiceDates,
				BillingDocumentDataNew.InvoiceDates,
				BillingDocumentDataOld.InvoiceDatesDesc);
			BillingDocument.InvoiceListDatesTooltip = Formatter.createTooltipConcat(this.reviewScenario, BillingDocumentDataOld.InvoiceListDates,
				BillingDocumentDataNew.InvoiceListDates,
				BillingDocumentDataOld.InvoiceListDatesDesc);
			BillingDocument.Incoterms1Tooltip = Formatter.createTooltipConcat(this.reviewScenario, BillingDocumentDataOld.Incoterms1,
				BillingDocumentDataNew.Incoterms1,
				BillingDocumentDataOld.Incoterms1Desc);
			BillingDocument.Incoterms2Tooltip = Formatter.createTooltipText(this.reviewScenario, BillingDocumentDataOld.Incoterms2,
				BillingDocumentDataNew.Incoterms2,
				BillingDocumentDataOld.Incoterms2);
			BillingDocument.TermsofPaymentTooltip = Formatter.createTooltipConcat(this.reviewScenario, BillingDocumentDataOld.TermsOfPayment,
				BillingDocumentDataNew.TermsOfPayment,
				BillingDocumentDataOld.SalestermsOfPaymentDesc);
			BillingDocument.AccountAssignmentGroupTooltip = Formatter.createTooltipConcat(this.reviewScenario, BillingDocumentDataOld.AcctAssgmtGroup,
				BillingDocumentDataNew.AcctAssgmtGroup,
				BillingDocumentDataOld.AcctAssgmtGroupDesc);
		...
		bindTaxClassification: function() {
			if (this.resubmitScenario === true || this.reviewScenario === true) {
				...
				var dataModel = this.getView().getModel("DisplayReadCall").getData();
				var data = dataModel.N_TaxClassif.results;
				var TaxClassifications = this.getView().getModel("TaxClassificationComboSet").getData().results.d.results;
				for (var i = 0; i < TaxClassifications.length; i++) {
					for (var j = 0; j < data.length; j++) {
						if (TaxClassifications[i].Cuntry.toString().trim() === data[j].Aland.toString().trim() && TaxClassifications[i].Taxcategory.toString()
							.trim() === data[j].Tatyp.toString().trim()) {
							TaxClassifications[i].Taxclassification = data[j].Taxkd;
							//TaxClassifications[i].Taxname = data[j].Vtxtk;
							TaxClassifications[i].TaxclassificationDesc = data[j].Vtext;
						}
					}
				}
				this.getView().getModel("TaxClassificationComboSet").refresh();
		...
		bindDefaultPartnerFunctions2: function() {
			if (this.resubmitScenario === true || this.reviewScenario === true) {
				...
				for (var i = 0; i < DefaultPartnerFunction.length; i++) {
					for (var j = 0; j < PartnerFunctions.length; j++) {
						if (PartnerFunctions[j].PartnerFunctionSection === "D" && DefaultPartnerFunction[i].PartnerFunction === PartnerFunctions[j].PartnerFunction) {
							...
							//Tooltip		
							/*DefaultPartnerFunction[i].PartFunTooltip = PartnerFunctionsOld[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
									PartnerFunctionsOld[i].PartnerFunction,
								PartnerFunctions[i].PartnerFunction, 	PartnerFunctionsOld[i].PartnerFunction);*/
							DefaultPartnerFunction[i].NumTooltip = PartnerFunctionsOld[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
								PartnerFunctionsOld[i].Number,
								PartnerFunctions[i].Number, PartnerFunctionsOld[i].Number);
							DefaultPartnerFunction[i].NameTooltip = PartnerFunctionsOld[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
								PartnerFunctionsOld[i].Name,
								PartnerFunctions[i].Name, PartnerFunctionsOld[i].Name);
							DefaultPartnerFunction[i].PartDescTooltip = PartnerFunctionsOld[i] === undefined ? "" : Formatter.createTooltipText(this
								.reviewScenario, PartnerFunctionsOld[i].PartnerDescription,
								PartnerFunctions[i].PartnerDescription, PartnerFunctionsOld[i].PartnerDescription);
						}
					}
				}
				this.getView().getModel("DefaultGeneratedPartnerFnSet").refresh();
		...
		bindDefaultPartnerFunctions: function() {
			if (this.resubmitScenario === true || this.reviewScenario === true) {
				var PartnerFunctionsNew = this.InputData.d.startTypeINPUT.start.DOCustomerChange.SalesDataNew.PartnerFunctions.results;
				var PartnerFunctionsOld = this.InputData.d.startTypeINPUT.start.DOCustomerChange.SalesDataOld.PartnerFunctions.results;
				var DefaultPartnerFunctionOld = this.getView().getModel("DefaultGeneratedPartnerFnSet").getData().results.d.results;
				var DefaultPartnerFunctionNew = [];
				for (var i = 0; i < PartnerFunctionsOld.length; i++) {
					for (var j = 0; j < DefaultPartnerFunctionOld.length; j++) {
						if (PartnerFunctionsOld[i].PartnerFunction === DefaultPartnerFunctionOld[j].PartnerFunction) {
							var editable = DefaultPartnerFunctionOld[j].Editable;
							break;
						}
					}
					DefaultPartnerFunctionNew.push({
						AccountGroup: this.InputData.d.startTypeINPUT.start.DOCustomerChange.BPNew.CustomerBusinessArea.AccountGroup,
						Default: JSON.parse(PartnerFunctionsNew[i].Default),
						Editable: editable,
						Name: PartnerFunctionsNew[i].Name,
						Number: PartnerFunctionsNew[i].Number,
						PartnerDescription: PartnerFunctionsNew[i].PartnerDescription,
						PartnerFunction: PartnerFunctionsNew[i].PartnerFunction,
						PartnerName: PartnerFunctionsOld[i].PartnerFuncDesc,
						custNumberValueState: "None",
						custNumberValueStateText: "",
						NumState: this.reviewScenario !== true ? "None" : PartnerFunctionsNew[i].Number !== PartnerFunctionsOld[i].Number ? "Warning" : "None",
						NameState: this.reviewScenario !== true ? "None" : PartnerFunctionsNew[i].Name !== PartnerFunctionsOld[i].Name ? "Warning" : "None",
						PartDescState: this.reviewScenario !== true ? "None" : PartnerFunctionsNew[i].PartnerDescription !== PartnerFunctionsOld[i].PartnerDescription ?
							"Warning" : "None",
						DefaultState: this.reviewScenario !== true ? "None" : PartnerFunctionsNew[i].Default !== PartnerFunctionsOld[i].Default ?
							"Warning" : "None",
						//Tooltip		
						NumTooltip: Formatter.createTooltipText(this.reviewScenario, PartnerFunctionsOld[i].Number, PartnerFunctionsNew[i].Number,
							PartnerFunctionsOld[i].Number),
						NameTooltip: Formatter.createTooltipText(this.reviewScenario, PartnerFunctionsOld[i].Name, PartnerFunctionsNew[i].Name,
							PartnerFunctionsOld[i].Name),
						PartDescTooltip: Formatter.createTooltipText(this.reviewScenario, PartnerFunctionsOld[i].PartnerDescription,
							PartnerFunctionsNew[i].PartnerDescription, PartnerFunctionsOld[i].PartnerDescription)
					});
				}
				this.getView().getModel("DefaultGeneratedPartnerFnSet").getData().results.d.results = DefaultPartnerFunctionNew;
				this.getView().getModel("DefaultGeneratedPartnerFnSet").refresh();
			}
		},
				...
				for (var i = 0; i < PartnerFunctions.length; i++) {
					if (PartnerFunctions[i].PartnerFunctionSection === "O") {
						OtherPartnerFunctions.push({
							...
							PartFunState: this.reviewScenario !== true ? "None" : PartnerFunctionsOld[i] === undefined ? "Warning" : PartnerFunctions[i].PartnerFunction !==
								PartnerFunctionsOld[i].PartnerFunction ? "Warning" : "None",
							PartFunMessage: " ",

							NumState: this.reviewScenario !== true ? "None" : PartnerFunctionsOld[i] === undefined ? "Warning" : PartnerFunctions[i].Number !==
								PartnerFunctionsOld[i].Number ? "Warning" : "None",
							NumMessage: " ",

							NameState: this.reviewScenario !== true ? "None" : PartnerFunctionsOld[i] === undefined ? "Warning" : PartnerFunctions[i].Name !==
								PartnerFunctionsOld[i].Name ? "Warning" : "None",
							NameMessage: "",

							PartDescState: this.reviewScenario !== true ? "None" : PartnerFunctionsOld[i] === undefined ? "Warning" : PartnerFunctions[i]
								.PartnerDescription !== PartnerFunctionsOld[i].PartnerDescription ? "Error" : "None",
							PartDescMessage: "",

							DefaultState: this.reviewScenario !== true ? "None" : PartnerFunctionsOld[i] === undefined ? "Warning" : PartnerFunctions[i].Default !==
								PartnerFunctionsOld[i].Default ? "Warning" : "None",
							DefaultMessage: "",

							//Tootip
							PartFunTooltip: PartnerFunctionsOld[i] === undefined ? "" : Formatter.createTooltipConcat(this.reviewScenario,
								PartnerFunctionsOld[i].PartnerFunction,
								PartnerFunctions[i].PartnerFunction, PartnerFunctionsOld[i].PartnerFuncDesc),
							NumTooltip: PartnerFunctionsOld[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
								PartnerFunctionsOld[i].Number,
								PartnerFunctions[i].Number, PartnerFunctionsOld[i].Number),
							NameTooltip: PartnerFunctionsOld[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario, PartnerFunctionsOld[
									i].Name,
								PartnerFunctions[i].Name, PartnerFunctionsOld[i].Name),
							PartDescTooltip: PartnerFunctionsOld[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
								PartnerFunctionsOld[i].PartnerDescription,
								PartnerFunctions[i].PartnerDescription, PartnerFunctionsOld[i].PartnerDescription)
						});
				...
				for (var i = 0; i < PartnerFunctions.length; i++) {
					if (PartnerFunctions[i].PartnerFunctionSection === "L") {
						SoldtoPartnerFunctions.push({
							...
							PartDescState: this.reviewScenario !== true ? "None" : PartnerFunctionsOld[i] === undefined ? "Warning" : PartnerFunctions[i]
								.PartnerDescription !== PartnerFunctionsOld[i].PartnerDescription ? "Warning" : "None",
							PartDescMessage: "",
							...
							PartFunTooltip: PartnerFunctionsOld[i] === undefined ? "" : Formatter.createTooltipConcat(this.reviewScenario,
								PartnerFunctionsOld[i].PartnerFunction,
								PartnerFunctions[i].PartnerFunction, PartnerFunctionsOld[i].PartnerFuncDesc),
							NumTooltip: PartnerFunctionsOld[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario, PartnerFunctionsOld[
									i].Number,
								PartnerFunctions[i].Number, PartnerFunctionsOld[i].Number),
							NameTooltip: PartnerFunctionsOld[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario, PartnerFunctionsOld[
									i].Name,
		...
		bindAdditionalFields: function() {
			...
			///////////////ToolTip/////////
			///////////////ToolTip/////////
			AdditionalFields.CustomerGroup1Tooltip = Formatter.createTooltipConcat(this.reviewScenario, AdditionalFieldsDataOld.CustomerGroup1,
				AdditionalFieldsDataNew.CustomerGroup1,
				AdditionalFieldsDataOld.CustomerGroup1Desc);
			AdditionalFields.CustomerGroup2Tooltip = Formatter.createTooltipConcat(this.reviewScenario, AdditionalFieldsDataOld.CustomerGroup2,
				AdditionalFieldsDataNew.CustomerGroup2,
				AdditionalFieldsDataOld.CustomerGroup2Desc);
			AdditionalFields.CustomerGroup3Tooltip = Formatter.createTooltipConcat(this.reviewScenario, AdditionalFieldsDataOld.CustomerGroup3,
				AdditionalFieldsDataNew.CustomerGroup3,
				AdditionalFieldsDataOld.CustomerGroup3Desc);
			AdditionalFields.CustomerGroup4Tooltip = Formatter.createTooltipConcat(this.reviewScenario, AdditionalFieldsDataOld.CustomerGroup4,
				AdditionalFieldsDataNew.CustomerGroup4,
				AdditionalFieldsDataOld.CustomerGroup4Desc);
			AdditionalFields.CustomerGroup5Tooltip = Formatter.createTooltipConcat(this.reviewScenario, AdditionalFieldsDataOld.CustomerGroup5,
				AdditionalFieldsDataNew.CustomerGroup5,
				AdditionalFieldsDataOld.CustomerGroup5Desc);
		...
		bindStatusData: function() {
			if (this.resubmitScenario === true || this.reviewScenario === true) {
				...
				///////////// ToolTip //////////////////
				StatusDataSet.BlockData.OrderBlockTooltip = Formatter.createTooltipConcat(this.reviewScenario, StatusDataOld.BlockData.OrderBlock,
					StatusDataNew.BlockData.OrderBlock,
					StatusDataOld.BlockData.OrderBlockDesc);
				StatusDataSet.BlockData.DeliveryBlockTooltip = Formatter.createTooltipConcat(this.reviewScenario, StatusDataOld.BlockData.DeliveryBlock,
					StatusDataNew.BlockData.DeliveryBlock,
					StatusDataOld.BlockData.DeliveryBlockDesc);
				StatusDataSet.BlockData.BillingBlockTooltip = Formatter.createTooltipConcat(this.reviewScenario, StatusDataOld.BlockData.BillingBlock,
					StatusDataNew.BlockData.BillingBlock,
					StatusDataOld.BlockData.BillingBlockDesc);
				this.getView().getModel("StatusDetailsSet").refresh();
			}
		},
			...
			if (this.resubmitScenario === true || this.reviewScenario === true) {
				if (this.InputData.d.startTypeINPUT.start.DOCustomerChange.TextNotesNew !== null) {
					...
					for (var i = 0; i < TextNotesData.results.length; i++) {
						var TextNotesDataNew = TextNotesData.results[i];
						if (this.InputData.d.startTypeINPUT.start.DOCustomerChange.TextNotesOld !== null) {
							var TextNotesDataOld = this.InputData.d.startTypeINPUT.start.DOCustomerChange.TextNotesOld.Text.results[i];
						} else {
							TextNotesDataOld = undefined;
						}
						...
						TextNotes.push({
							"Level": TextNotesDataNew.Level,
							"Language": TextNotesDataNew.Language,
							"TextType": TextNotesDataNew.Text_Type,
							"FreeText": TextNotesDataNew.Free_Text,
							"TextTypeArray": TextTypeArray,
							"LevelState": this.reviewScenario !== true ? "None" : TextNotesDataOld === undefined ? "Warning" : TextNotesDataNew.Level !==
								TextNotesDataOld.Level ? "Warning" : "None",
							"LanguageState": this.reviewScenario !== true ? "None" : TextNotesDataOld === undefined ? "Warning" : TextNotesDataNew.Language !==
								TextNotesDataOld.Language ? "Warning" : "None",
							"TextTypeState": this.reviewScenario !== true ? "None" : TextNotesDataOld === undefined ? "Warning" : TextNotesDataNew.Text_Type !==
								TextNotesDataOld.Text_Type ? "Warning" : "None",
							"FreeTextState": this.reviewScenario !== true ? "None" : TextNotesDataOld === undefined ? "Warning" : TextNotesDataNew.Free_Text !==
								TextNotesDataOld.Free_Text ? "Warning" : "None",
							///////////////ToolTip////////////////
							"LevelTooltip": TextNotesDataOld === undefined ? "" : Formatter.createTooltipText(this.reviewScenario, TextNotesDataOld.Level,
								TextNotesDataNew.Level, TextNotesDataOld.Level),
							"LanguageTooltip": TextNotesDataOld === undefined ? "" : Formatter.createTooltipText(this.reviewScenario, TextNotesDataOld.LanguageDesc,
								TextNotesDataNew.LanguageDesc, TextNotesDataOld.LanguageDesc),
							"TextTypeTooltip": TextNotesDataOld === undefined ? "" : Formatter.createTooltipText(this.reviewScenario, TextNotesDataOld.Text_Type,
								TextNotesDataNew.Text_Type, TextNotesDataOld.Text_Type),
							"FreeTextTooltip": TextNotesDataOld === undefined ? "" : Formatter.createTooltipText(this.reviewScenario, TextNotesDataOld.Free_Text,
								TextNotesDataNew.Free_Text, TextNotesDataOld.Free_Text)
						});
					}
			...
			if (this.resubmitScenario === true || this.reviewScenario === true) {
				if (this.InputData.d.startTypeINPUT.start.DOCustomerChange.TextNotesNew !== null) {
					...
					for (var i = 0; i < TextClassificationData.results.length; i++) {
						var TextClassificationDataNew = TextClassificationData.results[i];
						if (this.InputData.d.startTypeINPUT.start.DOCustomerChange.TextNotesOld !== null) {
							var TextClassificationDataOld = this.InputData.d.startTypeINPUT.start.DOCustomerChange.TextNotesOld.TextClassification.results[
								i];
						} else {
							TextClassificationDataOld = undefined;
						}
						TextClassification.push({
							"ClassificationType": TextClassificationDataNew.Classification_Type,
							"Classification": TextClassificationDataNew.Classification,
							"Value": TextClassificationDataNew.Value,
							"ClassificationTypeMessage": "",
							"ClassificationTypeState": this.reviewScenario !== true ? "None" : TextClassificationDataOld === undefined ? "Warning" : TextClassificationDataNew
								.Classification_Type !== TextClassificationDataOld.Classification_Type ? "Warning" : "None",
							"ClassificationMessage": "",
							"ClassificationState": this.reviewScenario !== true ? "None" : TextClassificationDataOld === undefined ? "Warning" : TextClassificationDataNew
								.Classification !== TextClassificationDataOld.Classification ? "Warning" : "None",
							"ValueMessage": "",
							"ValueState": this.reviewScenario !== true ? "None" : TextClassificationDataOld === undefined ? "Warning" : TextClassificationDataNew
								.Value !== TextClassificationDataOld.Value ? "Warning" : "None",
							"ClassiFicationArray": ClassiFicationArray,
							"ValueArray": ValueArray,
							//////////// ToolTip///////////
							"ClassificationTypeTooltip": this.reviewScenario !== true ? "" : TextClassificationDataOld === undefined ? "" : TextClassificationDataNew
								.Classification_Type !== TextClassificationDataOld.Classification_Type ? TextClassificationDataOld.Classification_Type : "",

							"ClassificationTooltip": this.reviewScenario !== true ? "" : TextClassificationDataOld === undefined ? "" : TextClassificationDataNew
								.Classification !== TextClassificationDataOld.Classification ? TextClassificationDataOld.Classification : "",
							"ValueTooltip": this.reviewScenario !== true ? "" : TextClassificationDataOld === undefined ? "" : TextClassificationDataNew
								.Value !== TextClassificationDataOld.Value ? TextClassificationDataOld.Value : ""
						});
					}
			...
			if (ContactPersons.results.length > 0) {
				...
				for (var i = 0; i < ContactPersons.results.length; i++) {
					...
					ContactPersonsData.ContactPersonList.push({
						...
						//////////////// tool tip//////////
						LastNameTooltip: ContactPersonsOld.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							ContactPersonsOld.results[i].Last_Name, ContactPersons
							.results[i].Last_Name, ContactPersonsOld.results[i].Last_Name),
						FunctionTooltip: ContactPersonsOld.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							ContactPersonsOld.results[i].Function, ContactPersons
							.results[i].Function, ContactPersonsOld.results[i].ContactpersonFuncDesc),
						TelephoneTooltip: ContactPersonsOld.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							ContactPersonsOld.results[i].Telephone, ContactPersons
							.results[i].Telephone, ContactPersonsOld.results[i].Telephone),
						FaxTooltip: ContactPersonsOld.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							ContactPersonsOld.results[i].Fax, ContactPersons
							.results[i].Fax, ContactPersonsOld.results[i].Fax),
						MobilePhoneTooltip: ContactPersonsOld.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							ContactPersonsOld.results[i].Mobile_Phone, ContactPersons
							.results[i].Mobile_Phone, ContactPersonsOld.results[i].Mobile_Phone),
						EmailTooltip: ContactPersonsOld.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							ContactPersonsOld.results[i].Email, ContactPersons
							.results[i].Email, ContactPersonsOld.results[i].Email),
						MethodTooltip: ContactPersonsOld.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							ContactPersonsOld.results[i].Method, ContactPersons
							.results[i].Method, ContactPersonsOld.results[i].MethodDesc),
						FirstNameTooltip: ContactPersonsOld.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							ContactPersonsOld.results[i].First_Name, ContactPersons
							.results[i].First_Name, ContactPersonsOld.results[i].First_Name),
						DepartmentTooltip: ContactPersonsOld.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							ContactPersonsOld.results[i].Department, ContactPersons
							.results[i].Department, ContactPersonsOld.results[i].ContactpersonDeptDesc),
						TelepExtensionTooltip: ContactPersonsOld.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							ContactPersonsOld.results[i].Telephone_Extension, ContactPersons
							.results[i].Telephone_Extension, ContactPersonsOld.results[i].Telephone_Extension),
						FaxExtensionTooltip: ContactPersonsOld.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							ContactPersonsOld.results[i].Fax_Extension, ContactPersons
							.results[i].Fax_Extension, ContactPersonsOld.results[i].Fax_Extension),
						NotesTooltip: ContactPersonsOld.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							ContactPersonsOld.results[i].Notes, ContactPersons
							.results[i].Notes, ContactPersonsOld.results[i].Notes),
						LanguageTooltip: ContactPersonsOld.results[i] === undefined ? "" : Formatter.createTooltipConcat(this.reviewScenario,
							ContactPersonsOld.results[i].Language, ContactPersons
							.results[i].Language, ContactPersonsOld.results[i].ContactpersonLangDesc),
		...
		bindTelephoneList: function() {
			...
			for (var i = 0; i < TelephoneSegment.results.length; i++) {
				if (TelephoneSegment.results[i].Default === "true") {
					...
					TelephoneData.ExtensionTooltip = TelephoneOldSegment.results[i] === undefined ? "" :
						Formatter.createTooltipText(this.reviewScenario, TelephoneOldSegment.results[i].Extension,
							TelephoneSegment.results[i].Extension, TelephoneOldSegment.results[i].Extension);

					TelephoneData.CommentsTooltip = TelephoneOldSegment.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
						TelephoneOldSegment.results[i].Comments,
						TelephoneSegment.results[i].Comments, TelephoneOldSegment.results[i].Comments);
				...
				TelephoneData.TelephoneList.push({
					...
					ExtensionTooltip: TelephoneOldSegment.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
						TelephoneOldSegment
						.results[i].Extension,
						TelephoneSegment.results[i].Extension, TelephoneOldSegment.results[i].Extension),
					...
					CommentsTooltip: TelephoneOldSegment.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
						TelephoneOldSegment.results[
							i].Comments,
						TelephoneSegment.results[i].Comments, TelephoneOldSegment.results[i].Comments),
		...
		bindFaxList: function() {
			...
			for (var i = 0; i < FaxSegment.results.length; i++) {
				if (FaxSegment.results[i].Default === "true") {
					...
					FaxData.ExtensionTooltip = FaxOldSegment.results[i] === undefined ? "" : Formatter.createTooltipText(
						this.reviewScenario, FaxOldSegment.results[
							i].Extension,
						FaxSegment.results[i].Extension, FaxOldSegment.results[i].Extension);
					...
					FaxData.CommentsTooltip = FaxOldSegment.results[i] === undefined ? "" : Formatter.createTooltipText(
						this.reviewScenario, FaxOldSegment.results[
							i].Comments,
						FaxSegment.results[i].Comments, FaxOldSegment.results[i].Comments);
				...
				FaxData.FaxList.push({
					...
					ExtensionTooltip: FaxOldSegment.results[i] === undefined ? "" : Formatter.createTooltipText(
						this.reviewScenario, FaxOldSegment.results[i].Extension,
						FaxSegment.results[i].Extension, FaxOldSegment.results[i].Extension),
					...
					CommentsTooltip: FaxOldSegment.results[i] === undefined ? "" : Formatter.createTooltipText(
						this.reviewScenario, FaxOldSegment.results[i].Comments,
						FaxSegment.results[i].Comments, FaxOldSegment.results[i].Comments),
		...
		bindEmailList: function() {
			...
			for (var i = 0; i < EmailSegment.results.length; i++) {
				if (EmailSegment.results[i].Default === "true") {
					...
					EmailData.CommentsTooltip = EmailOldSegment.results[i] === undefined ? "" : Formatter.createTooltipText(
						this.reviewScenario, EmailOldSegment.results[i].Comments, EmailSegment.results[i].Comments, EmailOldSegment.results[i].Comments
					);
				...
				EmailData.EmailList.push({
					...
					*/
					...
					CommentsTooltip: EmailOldSegment.results[i] === undefined ? "" : Formatter.createTooltipText(
						this.reviewScenario, EmailOldSegment.results[i].Comments, EmailSegment.results[i].Comments, EmailOldSegment.results[i].Comments
					),
		...
		bindURLList: function() {
			...
			for (var i = 0; i < URLSegment.results.length; i++) {
				if (URLSegment.results[i].Default === "true") {
					...
					URLData.CommentsTooltip = URlOldSegment.results[i] === undefined ? "" : Formatter.createTooltipText(
						this.reviewScenario, URlOldSegment.results[i].Comments, URLSegment.results[i].Comments, URlOldSegment.results[i].Comments);
				...
				URLData.URLList.push({
					...
					CommentsTooltip: URlOldSegment.results[i] === undefined ? "" : Formatter.createTooltipText(
						this.reviewScenario, URlOldSegment.results[i].Comments, URLSegment.results[i].Comments, URlOldSegment.results[i].Comments),
		...
		bindGeneralData: function() {
			...
			GenData.AltBusiNameMsg = this.reviewScenario !== true ? "None" : GeneralDataNew.AlternateBusinessName !== GeneralDataOld.AlternateBusinessName ?
				"Warning" : "None";
			GenData.AltBusiNameTooltip = Formatter.createTooltipText(this.reviewScenario,
				GeneralDataOld.AlternateBusinessName, GeneralDataNew.AlternateBusinessName, GeneralDataOld.AlternateBusinessName);
			GenData.SearchTearm1Msg = this.reviewScenario !== true ? "None" : GeneralDataNew.SearchTerm1 !== GeneralDataOld.SearchTerm1 ?
				"Warning" : "None";
			GenData.SearchTearm1Tooltip = Formatter.createTooltipText(this.reviewScenario,
				GeneralDataOld.SearchTerm1, GeneralDataNew.SearchTerm1, GeneralDataOld.SearchTerm1);
			GenData.SearchTearm2Msg = this.reviewScenario !== true ? "None" : GeneralDataNew.SearchTerm2 !== GeneralDataOld.SearchTerm2 ?
				"Warning" : "None";
			GenData.SearchTearm2Tooltip = Formatter.createTooltipText(this.reviewScenario,
				GeneralDataOld.SearchTerm2, GeneralDataNew.SearchTerm2, GeneralDataOld.SearchTerm2);
			GenData.CommuLangMsg = this.reviewScenario !== true ? "None" : GeneralDataNew.CommunicationLanguage !== GeneralDataOld.CommunicationLanguage ?
				"Warning" : "None";
			GenData.CommuLangTooltip = Formatter.createTooltipConcat(this.reviewScenario,
				GeneralDataOld.CommunicationLanguage, GeneralDataNew.CommunicationLanguage, GeneralDataOld.CommLangDesc);
			GenData.AuthoGrpMsg = this.reviewScenario !== true ? "None" : GeneralDataNew.AuthorizationGroup !== GeneralDataOld.AuthorizationGroup ?
				"Warning" : "None";
			GenData.AuthoGrpTooltip = Formatter.createTooltipText(this.reviewScenario,
				GeneralDataOld.AuthorizationGroup, GeneralDataNew.AuthorizationGroup, GeneralDataOld.AuthorizationGroup);
			GenData.VendNumMsg = this.reviewScenario !== true ? "None" : GeneralDataNew.VendorNumber !== GeneralDataOld.VendorNumber ?
				"Warning" : "None";
			GenData.VendNumTooltip = Formatter.createTooltipText(this.reviewScenario,
				GeneralDataOld.VendorNumber, GeneralDataNew.VendorNumber, GeneralDataOld.VendorNumber);
			GenData.TranspoZoneMsg = this.reviewScenario !== true ? "None" : GeneralDataNew.TransportationZone !== GeneralDataOld.TransportationZone ?
				"Warning" : "None";
			GenData.TranspoZoneTooltip = Formatter.createTooltipConcat(this.reviewScenario,
				GeneralDataOld.TransportationZone, GeneralDataNew.TransportationZone, GeneralDataOld.TransportationZoneDesc);
			...
			GenData.AltTranspoZoneMsg = this.reviewScenario !== true ? "None" : GeneralDataNew.AlternateTransportationZone !== GeneralDataOld.AlternateTransportationZone ?
				"Warning" : "None";
			GenData.AltTranspoZoneTooltip = Formatter.createTooltipConcat(this.reviewScenario,
				GeneralDataOld.AlternateTransportationZone, GeneralDataNew.AlternateTransportationZone, GeneralDataOld.AltTransportationZoneDesc
			);
			GenData.RegStructGrpMsg = this.reviewScenario !== true ? "None" : GeneralDataNew.Reg_Struct_Group !== GeneralDataOld.Reg_Struct_Group ?
				"Warning" : "None";
			GenData.RegStructGrpTooltip = Formatter.createTooltipText(this.reviewScenario,
				GeneralDataOld.Reg_Struct_Group, GeneralDataNew.Reg_Struct_Group, GeneralDataOld.Reg_Struct_Group);
			GenData.GroupKeyMsg = this.reviewScenario !== true ? "None" : GeneralDataNew.GroupKey !== GeneralDataOld.GroupKey ? "Warning" :
				"None";
			GenData.GroupKeyTooltip = Formatter.createTooltipConcat(this.reviewScenario,
				GeneralDataOld.GroupKey, GeneralDataNew.GroupKey, GeneralDataOld.GroupKeyDesc);
			GenData.TradingPartnerMsg = this.reviewScenario !== true ? "None" : GeneralDataNew.Trading_Partner !== GeneralDataOld.Trading_Partner ?
				"Warning" : "None";
			GenData.TradingPartnerTooltip = Formatter.createTooltipText(this.reviewScenario,
				GeneralDataOld.Trading_Partner, GeneralDataNew.Trading_Partner, GeneralDataOld.Trading_Partner);
			...
			GenData.LocationNo1Msg = this.reviewScenario !== true ? "None" : GeneralDataNew.Location_No1 !== GeneralDataOld.Location_No1 ?
				"Warning" : "None";
			GenData.LocationNo1Tooltip = Formatter.createTooltipText(this.reviewScenario,
				GeneralDataOld.Location_No1, GeneralDataNew.Location_No1, GeneralDataOld.Location_No1);
			GenData.LocationNo2Msg = this.reviewScenario !== true ? "None" : GeneralDataNew.Location_No2 !== GeneralDataOld.Location_No2 ?
				"Warning" : "None";
			GenData.LocationNo2Tooltip = Formatter.createTooltipText(this.reviewScenario,
				GeneralDataOld.Location_No2, GeneralDataNew.Location_No2, GeneralDataOld.Location_No2);
			GenData.CheckDegitMsg = this.reviewScenario !== true ? "None" : GeneralDataNew.CheckDigit !== GeneralDataOld.CheckDigit ? "Warning" :
				"None";
			GenData.CheckDegitTooltip = Formatter.createTooltipText(this.reviewScenario,
				GeneralDataOld.CheckDigit, GeneralDataNew.CheckDigit, GeneralDataOld.CheckDigit);
			...
			if (BankDataNew.BankDetails.results.length > 0) {
				...
				for (var i = 0; i < BankDataNew.BankDetails.results.length; i++) {
					...
					BankDetailsData.BankIBan.push({
						...
						IBanTooltip: BankDataOld.BankDetails.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							BankDataOld.BankDetails.results[i].IBAN, BankDataNew.BankDetails.results[i].IBAN, BankDataOld.BankDetails.results[i].IBAN),
						...
						CountryTooltip: BankDataOld.BankDetails.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							BankDataOld.BankDetails.results[i].Country, BankDataNew.BankDetails.results[i].Country, BankDataOld.BankDetails.results[i].Countryname
						),
						...
						BankKeyTooltip: BankDataOld.BankDetails.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							BankDataOld.BankDetails.results[i].BankKey, BankDataNew.BankDetails.results[i].BankKey, BankDataOld.BankDetails.results[i].BankKey
						),
						...
						BankControlKeyTooltip: BankDataOld.BankDetails.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							BankDataOld.BankDetails.results[i].BankControlKey, BankDataNew.BankDetails.results[i].BankControlKey, BankDataOld.BankDetails
							.results[i].BankControlKey),
						...
						BankAccountTooltip: BankDataOld.BankDetails.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							BankDataOld.BankDetails.results[i].BankAccount, BankDataNew.BankDetails.results[i].BankAccount, BankDataOld.BankDetails.results[
								i].BankAccount),
						...
						BankAccHolderTooltip: BankDataOld.BankDetails.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							BankDataOld.BankDetails.results[i].BankAccountHolder, BankDataNew.BankDetails.results[i].BankAccountHolder, BankDataOld.BankDetails
							.results[i].BankAccountHolder),
						...
						BankTypeTooltip: BankDataOld.BankDetails.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							BankDataOld.BankDetails.results[i].BankType, BankDataNew.BankDetails.results[i].BankType, BankDataOld.BankDetails.results[i]
							.BankType),
						...
						RefDetailsTooltip: BankDataOld.BankDetails.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							BankDataOld.BankDetails.results[i].ReferenceDetails, BankDataNew.BankDetails.results[i].ReferenceDetails, BankDataOld.BankDetails
							.results[i].ReferenceDetails),
			...
			if (UnloadingPoints.results.length > 0) {
				...
				for (var i = 0; i < UnloadingPoints.results.length; i++) {
					...
					CustomerDataModel.getData().UnloadingPoint.UnloadPointArr.push({
						...
						UnloadPointTooltip: UnloadingPointsOld.results[i] === undefined ? "" : Formatter.createTooltipText(this.reviewScenario,
							UnloadingPointsOld.results[i].UnloadPoint, UnloadingPoints.results[
								i].UnloadPoint, UnloadingPointsOld.results[i].UnloadPoint),
						...
						GoodsRecHrsTooltip: UnloadingPointsOld.results[i] === undefined ? "" : Formatter.createTooltipConcat(this.reviewScenario,
							UnloadingPointsOld.results[i].GRHours, UnloadingPoints.results[
								i].GRHours, UnloadingPointsOld.results[i].GoodsrecText),
						...
						CalenderKeyTooltip: UnloadingPointsOld.results[i] === undefined ? "" : Formatter.createTooltipConcat(this.reviewScenario,
							UnloadingPointsOld.results[i].CalendarKey, UnloadingPoints.results[
								i].CalendarKey, UnloadingPointsOld.results[i].CalendarKeyDesc),
