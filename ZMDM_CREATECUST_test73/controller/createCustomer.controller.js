sap.ui.define([
	...
	return Controller.extend("MDMZCUSTOMER.controller.createCustomer", {
		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf MDMZCUSTOMER.view.createCustomer
		 */
		onInit: function() {
		...
		_handleRouteMatched: function(oEvent) {
			if (oEvent.getParameter("name") === "createCustomer") {
				...
				// if (this.TaskInboxVariable === undefined) {
				var modelSearch = sap.ui.getCore().getModel("SearchMainModelSet");
		...
		readHdrAndCustBusAreaCreateEmpty: function() {
			//for duplicate check
			var modelMainDup = new sap.ui.model.json.JSONModel({
				DuplicateBtnEnable: false,
				NeedToValidateDuplicateCheck: true
			});
		...
		readSystemBusArea: function() {
			...
			if (this.reviewScenario === false) {
				var Filter = new sap.ui.model.Filter("URL", "EQ", "/RESTAdapter/UserAuthorization_MDM/MDMRefTable/ITOLOGICALCOMP?UserCode1=" + sap
					.ui
					.getCore().getModel("UserInfoSet").getData().UserCode1 + "" + compCode + "" + salesOrg);
			} else {
				return;
		...
		onSelectCompanyCode: function(oEvent) {
			...
			if (oEvent.getSource().getSelectedItem() !== null) {
				...
				//this.onAccountingData(selObj.companyCode, selObj.logicalCompId);
		...
		onChnageIconTabBar: function(oEvent) {
			...
			if (oEvent.getParameters().section.sId.split("--").pop() !== "goalsSection") {
			...
			if (tabChangeMdl.getData().SelecteIconTab === "ID_OP_TEXT_NOTE" && !this.firstTimeReadTextNoteIconTab) {
		...
		onPressSendForReview: function(oEvent) {
			...
			if (errCount > 0) {
				...
				this.onPressSubmitCustomerCreate("Review");
			}
		},
		...
		//Text Note
		textNoteTabInitialModelCreate: function() {
			var data2 = {
				TextNotes: {
					TextNotes2: [{
							Level: "",
		...
		onTextNotesLangguage: function(Sytem) {

			var that = this;
			var oBusyDialog = new sap.m.BusyDialog();
			oBusyDialog.open();
			var Filter1 = new sap.ui.model.Filter('URL', 'EQ',
				"/RESTAdapter/REST_SAP/ZSD_MDM_TEXTNOTES_API_SRV/Input_HelpSet?System_Name=" + Sytem + "&expand=NavLanguage");
				...
				"/BusinessPartnerSet", {
					...
					success: function(oData, oResponse) {
						...
						/*	var oGArray = [];
							var oG2;
							var oGText;
							var oG = JSON.parse(oData.results[0].Response).d.results[0].NavTextType.results;

							for (var c1 = 0; c1 < oG.length; c1++) {
							...
							var oGTModel = new sap.ui.model.json.JSONModel({

								"results": oGArray
							});
							that.getView().setModel(oGTModel, "generalTextSet");*/
						// var oAcountingText = JSON.parse(oData.results[0].Response).d.results[1].NavTextType.results;

						/*var oAArray = [];
						var oA2;
						var oAText;
						var oA = JSON.parse(oData.results[0].Response).d.results[1].NavTextType.results;

						for (var c2 = 0; c2 < oA.length; c2++) {
						...
						that.getView().setModel(oATModel, "acountingTextSet");
						// var oSalesText = JSON.parse(oData.results[0].Response).d.results[2].NavTextType.results;
						var oSArray = [];
						var oS2;
						var oSText;
						var oS = JSON.parse(oData.results[0].Response).d.results[2].NavTextType.results;

						for (var c3 = 0; c3 < oS.length; c3++) {
						...
						that.getView().setModel(oSTModel, "salesTextSet");*/
		...
		base64coonversionMethod: function(fileMime, fileName, fileDetails, fileSize, userName) {
			...
			reader.onload = function(readerEvt) {
				...
				tableData.push({
					...
					"FullName": userName,
					"TimeStamp": new Date(),
					"NoteEnabled": true, //Resubmit & Review scenario
					"DeleteEnabled": true //Resubmit & Review scenario
		...
		onPressSubmitCustomerCreate: function(ReqId) {
			var that = this;
			var payload = {
				BusinessPartnerData: {},
				CustomerData: {},
			...
			if (externalIdent.DUNS.toString().trim() !== "") {
				payload.BusinessPartnerData.External_Identifier.ExternalIdentifier.push({
					"ExternalIdentifierType": "",
					"identifierTypeRowid": this.DunsUniqRowId,
			...
			if (externalIdent.ChamberOf_Commerce.toString().trim() !== "") {
				payload.BusinessPartnerData.External_Identifier.ExternalIdentifier.push({
					"ExternalIdentifierType": "",
					"identifierTypeRowid": this.ChemOfCommsRowId,
			...
			payload.CustomerData.GeneralData.bsnsPrtnrId = "";
			...
			if (payload.BusinessPartnerData.CustomerBusinessArea.CompanyCode !== "") {
				...
				payload.AccountingData.Correspondence.Last_Dunned = Formatter.dateSendToSap(corresp.LastDunned);
		...
		createAccountyEmptyModel: function() {
			var accData = {
				AccountManagement: {
					ReconAccount: "",
					HeadOffice: "",
		...
		onAccountingData: function(oEvent, system) {
			var that = this;
		...
		autoPopulateSystem: function() {
			...
			if (this.resubmitScenario === false && this.reviewScenario === false) {
				...
				this.showMessageBasedOnHeaderSelection();
				if (modelHdr.CompnyCodeKey !== "") {
					this.onAccountingData(modelHdr.CompnyCodeKey, modelHdr.System);
		...
		readTaxClassification: function() {
			...
			var URL =
				"/RESTAdapter/ACCOUNTING_DATA_FETCH/ZSD_MDM_SALES_DATA_API_SRV/TaxCategorySet?System_Name=" + hdrData.System +
				"&filter=Companycode eq '" +
		...
		createStatusDetailstab: function() {
			var statusDetails = {
				DeletionFlag: {
		...
		readDefaultPartnerFunctions: function(accountGroup) {
			...
			if (accountGroup !== "") {
				var URL =
					"/RESTAdapter/Default_Partner_Function/ZSD_MDM_SALES_DATA_API_SRV/DefaultPartnerFunctionSet?System_Name=" + hdrData.System +
		...
		readPartnerFunction: function(accountGroup) {
			...
			if (accountGroup !== "") {
				var URL =
					"/RESTAdapter/Other_Partner_Function/ZSD_MDM_SALES_DATA_API_SRV/InputHelp_OtherPartnerFunctionSet?System_Name=" + hdrData.System +
					"&filter=AccountGroup eq '" +
			...
			// var URL =
			// 	"/RESTAdapter/Other_Partner_Function/ZSD_MDM_SALES_DATA_API_SRV/InputHelp_OtherPartnerFunctionSet?System_Name="+hdrData.System+"&filter=AccountGroup eq '0001'&expand=NavOtherPartnerFunction";
			var Filter = new sap.ui.model.Filter('URL', 'EQ', URL);
			var oBusyDialog = new sap.m.BusyDialog();
			oBusyDialog.open();
			this.oModel.read(
		...
		basedOnAccountGroupEnableMandatoryOptional: function(keyAcc) {
			...
			var Filter1 = new sap.ui.model.Filter('URL', 'EQ',
				...
				"&filter=ImTcode eq 'CR' and ImAccgr eq '" +
		...
		displayCustomerRequestDetails: function() {
			//bind task details to UI JSON model
			this.bindCreateCustomerModel();
		},
		...
		bindCreateCustomerModel: function() {
			...
			this.onAccountingData(companyCode, system);
		...
		bindCustomerBusinessAreaModel: function() {
			...
			if (this.resubmitScenario === true) {
				hdr.SystemEnable = true;
			} else {
				hdr.SystemEnable = false;
			}
			this.getView().getModel("HeaderModelSet").refresh();
			//this.readSalesArea();
		},
		...
		bindNameAndAddress: function() {
			...
			for (var i = 0; i < BusinessPartnerData.Name_Address.results.length; i++) {
				...
				model.getData().NameAndAddress.push({
					...
					"POBoxCityState": "None",
					"POBoxCityMessage": " ",
					"PreferedAddrsVerEnabled": NameAndAddress.AddressVersion === "I" ? false : true, //false,
		...
		bindCorrespondence: function() {
			...
			CorrespondenceData.LastDunned = Formatter.readLastDunnedDateApproval(Correspondence.Last_Dunned); //;
		...
		updatePartnerFunctions: function(payload) {
			...
			this.InputData.d.startTypeINPUT.start.DO_DT_Fiori_Customer_Create.SalesData.results[0].PartnerFunctions.results = PartnerFunctions;
		},
